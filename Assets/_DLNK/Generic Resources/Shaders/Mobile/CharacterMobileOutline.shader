...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normalmap,TRANSFORM_TEX(node_338.rg, _Normalmap))).rgb;
...
/////// Diffuse:
                ...
                float4 node_233 = tex2D(_HeightOccTrans,TRANSFORM_TEX(node_338.rg, _HeightOccTrans));
                float4 node_58 = tex2D(_TintMask,TRANSFORM_TEX(node_338.rg, _TintMask));
...
////// Specular:
                ...
                float4 node_2 = tex2D(_MainTex,TRANSFORM_TEX(node_338.rg, _MainTex));
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normalmap,TRANSFORM_TEX(node_339.rg, _Normalmap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/////// Diffuse:
                float NdotL = dot( normalDirection, lightDirection );
                float4 node_233 = tex2D(_HeightOccTrans,TRANSFORM_TEX(node_339.rg, _HeightOccTrans));
                float4 node_58 = tex2D(_TintMask,TRANSFORM_TEX(node_339.rg, _TintMask));
                float node_126 = (node_58.r*node_58.g*node_58.b);
                float3 node_143 = lerp(((_ColorCRGBGlossA.rgb*node_58.r)+(_ColorBRGBGlossA.rgb*node_58.g)+(_ColorARGBGlossA.rgb*node_58.b)),_ColorDRGBGlossA.rgb,node_126);
                float node_198 = lerp(((node_58.r*_ColorCRGBGlossA.a)+(node_58.g*_ColorBRGBGlossA.a)+(node_58.b*_ColorARGBGlossA.a)),_ColorDRGBGlossA.a,node_126);
                float3 w = (node_233.b*node_143*_TransPower*(1.0 - node_198))*0.5; // Light wrapping
...
////// Specular:
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float4 node_2 = tex2D(_MainTex,TRANSFORM_TEX(node_339.rg, _MainTex));
                finalColor += diffuseLight * lerp( node_2.rgb, (node_143*node_2.rgb*node_234), _TintEnable );
                finalColor += specular;
/// Final Color:
